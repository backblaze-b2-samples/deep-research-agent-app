...
const VENV_DIR = resolve(REPO_ROOT, "services/api/.venv");
const VENV_UVICORN = resolve(VENV_DIR, "bin/uvicorn");
...
function checkEnv() {
  ...
  if (placeholders.length > 0) {
    fail(
      ...
      "Edit .env and replace placeholders with your real B2 credentials (https://secure.backblaze.com/app_keys.htm?utm_source=github&utm_medium=referral&utm_campaign=ai_artifacts&utm_content=b2ai-deep-research-agent-app)",
    );
  }

  // Anthropic key powers the research agent — required, but reported with
  // its own message so the fix is obvious.
  const missingOther = REQUIRED_OTHER_VARS.filter((k) => !env[k]);
  if (missingOther.length > 0) {
    fail(
      `.env is missing required variables: ${missingOther.join(", ")}`,
      "Add ANTHROPIC_API_KEY to .env — get a key at https://console.anthropic.com/",
    );
  }
  const otherPlaceholders = REQUIRED_OTHER_VARS.filter(
    (k) => env[k] && PLACEHOLDERS.has(env[k]),
  );
  if (otherPlaceholders.length > 0) {
    fail(
      `.env still has placeholder values: ${otherPlaceholders.join(", ")}`,
      "Edit .env and replace ANTHROPIC_API_KEY with your real key from https://console.anthropic.com/",
    );
  }
}
...
// The agent's fetch_source tool renders pages with Playwright Chromium.
// Without the browser binary every fetch fails at runtime, so surface it
// here at preflight. Best-effort: if the venv isn't built yet we stay quiet
...
function checkChromium() {
  ...
  if (probe && /download|is not installed|Install location/i.test(probe)) {
    warn(
      ...
      "Run: `cd services/api && .venv/bin/python -m playwright install chromium`",
    );
  }
}
