...
The research agent makes real model calls and renders real web pages, so the
backend needs two things beyond the usual venv:

- **Anthropic key.** Put `ANTHROPIC_API_KEY` in the repo-root `.env`. The agent
  uses Claude Sonnet 4.6 with bundled web search. A full run costs ~$0.50–$1.10;
  tune down with `RESEARCH_MAX_SEARCHES` / `RESEARCH_MAX_SOURCES`.
- **Playwright Chromium.** After installing requirements, run
  `python -m playwright install chromium` inside `services/api/.venv`. `pnpm doctor`
  warns if Chromium is missing.
...
### Mocking the agent in tests

`pnpm test:api` must spend **no tokens and make no network calls**. Tests mock
the two external boundaries and exercise the real loop + B2 prefix logic against
an in-memory fake S3 client:

- Patch `service.research.llm_client.create_turn` to return canned
  `Message`-shaped objects (text blocks and `tool_use` blocks).
- Patch `service.research.browser.fetch_page` to return a `FetchedPage`.
- Patch `repo.b2_research.get_s3_client` with an in-memory fake (see
  `tests/test_research_service.py::FakeS3`).

Because the SDKs (`anthropic`, `playwright`, `trafilatura`, `httpx`) are
confined to `repo/`, mocking those two module attributes is enough to keep the
whole loop hermetic.
...
- E2E: `pnpm test:e2e` (run `pnpm --filter @deep-research-agent-app/web exec playwright install chromium` once first)
